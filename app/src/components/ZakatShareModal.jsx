...
// Same canvas-share shell as ReceiptShareModal.jsx/WarisShareModal.jsx —
// Kalkulator Zakat had no share option at all before this, despite its
// result already being shown as a nicely-styled card on screen.
export default function ZakatShareModal({ typeLabel, amountLabel, formulaLabel, theme, onClose }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setReady(false);
    drawZakatCard(canvasRef.current, { typeLabel, amountLabel, formulaLabel, theme }).then(() => {
      if (!cancelled) setReady(true);
    });
    return () => {
      cancelled = true;
    };
  }, [typeLabel, amountLabel, formulaLabel, theme]);
  ...
  async function handleShare() {
    setBusy(true);
    try {
      const file = await canvasToFile(canvasRef.current, 'hasil-zakat.png');
      await shareFile({ file, title: 'Hasil Kalkulator Zakat - airmoon', onFallback: handleDownload });
    } finally {
      setBusy(false);
    }
  }
