...
import { loadFavoriteMasjid, addFavoriteMasjid, removeFavoriteMasjid } from '../lib/favoriteMasjid';
import PullToRefresh from '../components/PullToRefresh';

const HISTORY_KEY = 'masjid';

function MosqueRow({ m, origin, isFav, onToggleFav }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 13, borderRadius: 16, background: 'var(--card)' }}>
      ...
      <button
        ...
        aria-label={isFav ? `Hapus ${m.name} dari favorit` : `Simpan ${m.name} sebagai favorit`}
        style={{ width: 34, height: 34, borderRadius: '50%', border: 'none', background: 'transparent', color: isFav ? 'var(--gold-ink)' : 'var(--muted)', fontSize: 17, cursor: 'pointer', flexShrink: 0 }}
      >
        {isFav ? '⭐' : '☆'}
      </button>
      <a
...
export default function CariMasjid() {
  ...
  const [favorites, setFavorites] = useState(loadFavoriteMasjid);

  useEffect(() => setHistory(getSearchHistory(HISTORY_KEY)), []);

  function toggleFavorite(m) {
    const isFav = favorites.some((f) => f.id === m.id);
    setFavorites(isFav ? removeFavoriteMasjid(m.id) : addFavoriteMasjid(m));
  }
  ...
  useEffect(() => {
    refresh();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- refresh is stable enough for this page's needs; retryTick alone should re-trigger it
  }, [retryTick]);
      ...
      <PullToRefresh onRefresh={refresh}>
        ...
        {!query && favorites.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {favorites.map((m) => (
              <MosqueRow key={m.id} m={m} origin={origin} isFav onToggleFav={toggleFavorite} />
            ))}
          </div>
        )}
        ...
        {status === 'ready' && filtered.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {filtered.map((m) => (
              <MosqueRow key={m.id} m={m} origin={origin} isFav={favorites.some((f) => f.id === m.id)} onToggleFav={toggleFavorite} />
            ))}
          </div>
        )}
      </PullToRefresh>
