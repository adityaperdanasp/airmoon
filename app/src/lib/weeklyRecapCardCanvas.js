// Draws a shareable "Capaian Mingguan" card — same plain Canvas 2D
// approach as the app's other share cards, summarizing the last 7 days'
// Amalan Harian points (from AmalanHeatmap.jsx's own already-fetched data,
// no separate query) rather than the lifetime totals Kartu Pencapaian/
// Ringkasan Ibadah already cover — a short-term motivation snapshot.
...
export async function drawWeeklyRecapCard(canvas, { totalScore, maxScore, bestDayScore, bestDayMax, theme = 'light' }) {
  canvas.width = W;
  canvas.height = H;
  ...
  const photoSrc = pool[(dayOfYear + 9) % pool.length]; // offset distinct from other day-rotated cards
  ...
  ctx.fillText('CAPAIAN MINGGUAN', W / 2, H * 0.32);

  ctx.font = '800 150px Poppins, sans-serif';
  ctx.fillStyle = '#ffffff';
  ctx.fillText(String(totalScore), W / 2, H * 0.46);

  ctx.font = '400 34px Poppins, sans-serif';
  ctx.fillStyle = 'rgba(244,240,230,0.9)';
  ctx.fillText(`dari ${maxScore} poin, 7 hari terakhir`, W / 2, H * 0.51);

  ctx.font = '600 28px Poppins, sans-serif';
  ctx.fillStyle = 'rgba(244,240,230,0.85)';
  ctx.fillText(`Hari terbaik: ${bestDayScore}/${bestDayMax} poin`, W / 2, H * 0.58);
