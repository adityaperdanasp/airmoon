...
// Same canvas-share shell as TasbihShareModal.jsx/ReceiptShareModal.jsx —
// Puasa Sunnah had no share option at all before this.
export default function PuasaSunnahShareModal({ totalCount, thisMonthCount, monthLabel, badgeLabel, theme, onClose }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setReady(false);
    drawPuasaSunnahCard(canvasRef.current, { totalCount, thisMonthCount, monthLabel, badgeLabel, theme }).then(() => {
      if (!cancelled) setReady(true);
    });
    return () => {
      cancelled = true;
    };
  }, [totalCount, thisMonthCount, monthLabel, badgeLabel, theme]);
  ...
  async function handleShare() {
    setBusy(true);
    try {
      const file = await canvasToFile(canvasRef.current, 'puasa-sunnah.png');
      await shareFile({ file, title: 'Progress Puasa Sunnah - airmoon', onFallback: handleDownload });
    } finally {
      setBusy(false);
    }
  }
