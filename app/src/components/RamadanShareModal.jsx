...
// Same shell as AmalanShareModal.jsx/AyatCardModal.jsx (canvas preview +
// Unduh/Bagikan) — Ramadan Mode's puasa/tarawih tracker had no share
// option at all despite tracking both all month.
export default function RamadanShareModal({ puasaCount, tarawihCount, monthDays, hijriYear, onClose }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setReady(false);
    drawRamadanCard(canvasRef.current, { puasaCount, tarawihCount, monthDays, hijriYear }).then(() => {
      if (!cancelled) setReady(true);
    });
    return () => {
      cancelled = true;
    };
  }, [puasaCount, tarawihCount, monthDays, hijriYear]);
  ...
  function handleDownload() {
    canvasRef.current.toBlob((blob) => {
      ...
      a.href = url;
      a.download = 'progress-ramadan.png';
      a.click();
  ...
  async function handleShare() {
    setBusy(true);
    try {
      const file = await canvasToFile(canvasRef.current, 'progress-ramadan.png');
      await shareFile({ file, title: 'Progress Ramadan - airmoon', onFallback: handleDownload });
    } finally {
      setBusy(false);
    }
  }
