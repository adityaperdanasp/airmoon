...
// Same canvas-card share shell as the app's other share modals, for a
// Tasbih Digital tally — see lib/tasbihCardCanvas.js.
export default function TasbihShareModal({ phraseLabel, phraseArab, count, laps, target, onClose }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setReady(false);
    drawTasbihCard(canvasRef.current, { phraseLabel, phraseArab, count, laps, target, theme }).then(() => {
      if (!cancelled) setReady(true);
    });
    return () => {
      cancelled = true;
    };
  }, [phraseLabel, phraseArab, count, laps, target, theme]);

  async function handleShare() {
    setBusy(true);
    try {
      const file = await canvasToFile(canvasRef.current, 'tasbih.png');
      await shareFile({ file, title: 'Tasbih Digital - airmoon', onFallback: handleDownload });
    } finally {
      setBusy(false);
    }
  }
