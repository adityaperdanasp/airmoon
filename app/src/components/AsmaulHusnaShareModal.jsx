...
// Same canvas-share shell as SejarahIslamShareModal.jsx — Asmaul Husna
// had no share capability at all before this.
export default function AsmaulHusnaShareModal({ no, arabic, latin, meaning, photoIndex, theme, onClose }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setReady(false);
    drawAsmaulHusnaCard(canvasRef.current, { no, arabic, latin, meaning, photoIndex, theme }).then(() => {
      if (!cancelled) setReady(true);
    });
    return () => {
      cancelled = true;
    };
  }, [no, arabic, latin, meaning, photoIndex, theme]);
  ...
  function handleDownload() {
    canvasRef.current.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `asmaul-husna-${latin.toLowerCase().replace(/[^a-z0-9]+/g, '-')}.png`;
      a.click();
  ...
  async function handleShare() {
    setBusy(true);
    try {
      const file = await canvasToFile(canvasRef.current, 'asmaul-husna.png');
      await shareFile({ file, title: `${latin} - airmoon`, onFallback: handleDownload });
    } finally {
      setBusy(false);
    }
  }
